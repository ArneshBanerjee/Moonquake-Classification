...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(3,
      ...
      selectInput("plot_type", "Select Plot Type:",
                  choices = c("Info" = "info",
                              ...
                              "Seismogram (Time Series)" = "seismogram",
      ...
      # Info page controls
      conditionalPanel(
        condition = "input.plot_type == 'info'",
        helpText("Welcome to the Moonquake Classification Dashboard")
      ),
    ...
    column(9,
      ...
      conditionalPanel(
        condition = "input.plot_type == 'info'",
        div(style = "text-align: center; padding: 50px; min-height: 600px; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); border-radius: 15px;",
            div(style = "animation: fadeInDown 0.5s ease-out;",
                h1(style = "color: #ff6b6b; margin-bottom: 30px; font-size: 3.5em; font-weight: bold; text-shadow: 2px 2px 4px rgba(0,0,0,0.3);", 
                   "Team 05: eRRoR 404")
            ),
            div(style = "animation: fadeInUp 1.5s ease-out;",
                h2(style = "color: #4ecdc4; margin-bottom: 40px; font-size: 2em;", 
                   "Project: Classification of Moonquakes using R")
            ),
            div(style = "animation: fadeIn 2s ease-out; background-color: rgba(255,255,255,0.1); padding: 30px; border-radius: 15px; margin: 0 auto; max-width: 500px;",
                h3(style = "color: #ff6b6b; margin-bottom: 20px; font-size: 1.8em;", "Team Members:"),
                div(style = "margin: 20px 0;",
                    h4(style = "color: #ffffff; margin: 15px 0; font-size: 1.4em; transition: transform 0.3s ease;", 
                       "Arnesh Banerjee")
                ),
                div(style = "margin: 20px 0;",
                    h4(style = "color: #ffffff; margin: 15px 0; font-size: 1.4em; transition: transform 0.3s ease;", 
            ...
            div(style = "margin-top: 40px; animation: fadeIn 2.5s ease-out;",
...
server <- function(input, output, session) {
  ...
  data <- reactive({
    tryCatch({
      df <- read_excel("/Users/arnesh/Developer/R-Moonquake/final.xlsx")
      return(df)
    }, error = function(e) {
      return(NULL)
    })
